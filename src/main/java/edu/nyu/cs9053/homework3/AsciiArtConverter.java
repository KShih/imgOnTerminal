...
public class AsciiArtConverter {
    ...
    public char[][] convert(ImageInfoProvider infoProvider) {
        ...
        char[][] returnCharArray = new char[height][width];
        for (int row = 0; row < height; ++row) {
            for (int col = 0; col < width; ++col) {
                double pixel = grayscaler.grayScale(infoProvider.getPixel(col, row));
                returnCharArray[row][col] = getCharacterForPixel(pixel);
            }
        }
    ...
    protected char getCharacterForPixel(double pixel) {
        // TODO - implement this
        double scaleByte = pixel / 255d;
        int lookupIndex = (int) Math.ceil(scaleByte * (ASCII_ART_LOOKUP_TABLE.length - 1));
        return ASCII_ART_LOOKUP_TABLE[lookupIndex];
